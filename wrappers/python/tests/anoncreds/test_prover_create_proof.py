...
@pytest.mark.asyncio
async def test_virtual_prover_create_proof_works(ew_wallet_handle, ew_prepopulated_wallet, gvt_schema, master_secret_name,
                                         schema_key):
    claim_def_json, _, _, _ = ew_prepopulated_wallet
    ...
    claims = json.loads(await prover_get_claims_for_proof_req(ew_wallet_handle, json.dumps(proof_req)))
    claim_for_attr = claims['attrs']['attr1_referent'][0]['referent']
    claim_for_predicate = claims['predicates']['predicate1_referent'][0]['referent']
    ...
    await prover_create_proof(ew_wallet_handle, json.dumps(proof_req), json.dumps(requested_claims),
                              json.dumps(schemas), master_secret_name,
                              json.dumps(claim_defs), "{}")
